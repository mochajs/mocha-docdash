...
function needsSignature(doclet) {
  ...
  if (
    ...
    (doclet.kind === 'class' && !doclet.hideconstructor)
  ) {
    needsSig = true;
  }
    ...
    if (symbols[module.longname]) {
      module.modules = symbols[module.longname]
        ...
        .map(function (symbol) {
          ...
          if (
            symbol.kind === 'class' ||
            (symbol.kind === 'function' && !symbol.hideconstructor)
          ) {
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
  ...
  if (items && items.length) {
    var itemsNav = '';
    var docdash =
      env && env.conf && (env.conf['mocha-docdash'] || env.conf.docdash || {});
    var level =
...
function buildNav(members) {
  ...
  if (members.globals.length) {
    ...
    members.globals.forEach(function (g) {
      if (
        (docdash.typedefs || g.kind !== 'typedef') &&
...
exports.publish = function (taffyData, opts, tutorials) {
  var docdash =
    env &&
    env.conf &&
    (env.conf.mochaDocdash ||
      env.conf['mocha-docdash'] ||
      env.conf.docdash ||
      {});
  data = taffyData;
